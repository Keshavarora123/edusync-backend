...
namespace EdySyncProject.Controllers
{
    ...
    public class AssessmentsController : ControllerBase
    {
        ...
        public async Task<ActionResult<AssessmentDTO>> PostAssessment([FromBody] CreateAssessmentDTO dto)
        {
            ...
            // Add the assessment to the context first to generate the ID
            _context.Assessments.Add(assessment);
            
            // Now create the questions with the assessment ID
            var questions = dto.Questions.Select(q => new Question
            {
                ...
                AssessmentId = assessment.AssessmentId // Explicitly set the foreign key
            }).ToList();
        ...
        public async Task<IActionResult> SubmitAssessment([FromBody] SubmitAssessmentDTO dto)
        {
            try
            {
                ...
                foreach (var question in assessment.Questions)
                {
                    ...
                    if (answer != null && string.Equals(answer.SelectedOption, question.CorrectOption, StringComparison.OrdinalIgnoreCase))
                    {
                        correctCount++;
                    }
                }
